...
$MyReport = Get-CustomHTML "$Server vCheck"
	...
	$MyReport += Get-CustomHeader ("This report took " + [math]::round(((Get-Date) - $Date).TotalMinutes,2) + " minutes to run all checks.")
...
if ($Outputpath) {
	$Filename = $Outputpath + "\" + $Server + "vCheck" + "_" + $Date.Day + "-" + $Date.Month + "-" + $Date.Year + ".htm"
	$MyReport | out-file -encoding ASCII -filepath $Filename
}
...
if ($SendEmail) {
	...
	If ($SendAttachment) {
		send-Mailmessage -To $EmailTo -From $EmailFrom -Subject "$Server vCheck Report"  -SmtpServer $SMTPSRV -Body "vCheck attached to this email" -Attachments $Filename
	} Else {
		send-Mailmessage -To $EmailTo -From $EmailFrom -Subject "$Server vCheck Report"  -SmtpServer $SMTPSRV -Body $MyReport -BodyAsHtml
	}
}
